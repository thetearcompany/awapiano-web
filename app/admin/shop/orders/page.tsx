...
export default function OrdersPage() {
  ...
  const { 
    ...
    isLoading, 
    refetch 
  } = trpc.admin.shop.getOrders.useQuery({
    limit: filters.limit,
  ...
  // Fetch shop stats
  const { data: shopStats } = trpc.admin.shop.getShopStats.useQuery()
  
  // Update order status mutation
  const updateOrderStatusMutation = trpc.admin.shop.updateOrderStatus.useMutation({
    onSuccess: () => {
      toast({
        title: "Order updated",
  ...
  const totalPages = Math.ceil((ordersData?.orders.length || 0) / filters.limit)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        {shopStats?.ordersByStatus.map((statusData) => (
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                ...
                <div className={`h-8 w-8 rounded-full flex items-center justify-center ${
                  statusData.status === OrderStatus.PENDING ? "bg-yellow-500/20" :
